...
server <- function(input, output, session) {
  ...
  input_vector <- reactive({
    if(!is.null(input$geneInput)) {
      ...
      print(paste0('Split input vector:', genes))
      return(genes)
    } else {
      return(NULL)
    }
  ...
  observeEvent(input$btn_generate, {
    ...
    p<- ggplot()
    df <- data.frame()
    grn_df <- data.frame(msg='Select inputs, then press generate results')
    ...
    if (input$experiment == "Lesion Size Correlation") {
      ...
      print(paste('time-series plotting args:',args))


      if(length(args)>0){
        #call `plot_timeseries_expr` func
        print('calling time-series plotting function')
